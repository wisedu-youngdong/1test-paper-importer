...
[가장 중요 — 듣기 영역 제외]
한국 수능/모의고사 영어 영역은 보통 1~17번이 듣기(Listening), 18번부터가 읽기(Reading)입니다.
듣기 문항은 지시문에 "다음을 듣고", "대화를 듣고", "남자의 말을 듣고" 같은 표현이 있거나,
지문 내용이 짧은 대화/안내방송 형태이고 그림(picture)을 고르는 문제 등의 특징이 있습니다.
이런 듣기 문항은 items 배열에 절대 포함하지 마세요. 오직 읽기(Reading) 지문 기반 문항만 분석하세요.
PDF에 듣기 문항이 섞여 있다면 건너뛰고, 읽기 문항부터 번호를 그대로 유지해서 분석하세요
(예: 18번부터 시작해도 items의 첫 number는 18이어야 하며, 1로 재배열하지 마세요).

1. 각 문항 번호(1번, 2번 등)를 찾아내세요
2. 각 문항의 유형을 아래 [공식 카테고리 체계]에서 정확히 하나를 골라 분류하세요. 목록에 없는 임의 명칭(예: "정보확인", "내용확인" 등 목록에 없는 줄임말)을 만들지 마세요. 만약 듣기 문항이라 25개 읽기 유형 중 어디에도 맞지 않는다면, 그건 위 [듣기 영역 제외] 규칙에 따라 아예 분석 대상에서 제외해야 한다는 신호입니다.
...
4. 각 문항의 지문 전체 원문을 빠짐없이 그대로 추출하세요 (요약하거나 생략하지 말고 PDF에 적힌 원문 그대로)
   ...
   - [매우 중요] fullPassage에는 순수 지문 본문만 넣으세요. 다음은 절대 fullPassage에 포함하지 마세요:
     ...
     · 문항 번호와 배점 표기 (예: "30.", "(3점)", "[3점]", "[2점]" 같은 점수 표시는
       지문 끝이든 중간이든 어디에 있든 전부 제거하고 fullPassage에 포함하지 마세요)
   ...
   - [문장요약 유형 전용 규칙] 유형이 "문장요약"인 경우, fullPassage 안에 지문 본문과
     요약문을 구분자 "[[SUMMARY]]"로 분리해서 넣으세요.
     요약문 안의 빈칸은 반드시 "(A)_______________" "(B)_______________" 형태로
     괄호+밑줄 12개 이상을 붙여서 빈칸임을 명확히 표시하세요 (괄호만 쓰고 끝내지 마세요).
     형식: "지문 본문 전체[[SUMMARY]]요약문 텍스트...and (A)_______________ their goals...a(n) (B)_______________ self-view."
